...
const Navbar = ({ sticky, border }) => {
  ...
  return (
    ...
    <nav
      className={`${(scrolled || border) && "border-b-2"} ${sticky && "sticky top-0"} relative z-50 transition-all duration-700 bg-white`}
    >
      {/* element gets a shadow when menu is toggled -> when menu is clicked and on display, the element gets a shadow  */}
      <div
        className={`container mx-auto w-full flex justify-between items-center px-4 py-3 ${isMenuToggled && "shadow-md"} transition-shadow duration-300`}
      >
        {/* Logo */}
        <Link to={"/"} className="transition-transform duration-300 hover:scale-105">
          <div className="flex items-center">
            ...
            <span className="ml-2 font-bold text-cyan-600 hidden sm:inline">Urban Explore</span>
          </div>
        </Link>
        {/*  */}
        ...
        <ul className="hidden mmd:flex space-x-1">
          ...
          <Link to={"/hotels"}>
            <li className="rounded-full hover:bg-gray-200 py-2 px-3 cursor-pointer transition-all duration-300 hover:scale-105">
          ...
          {/* Link to Map View */}
          <Link to={"/map"}>
            <li className="rounded-full bg-cyan-600 text-white py-2 px-3 cursor-pointer transition-all duration-300 hover:bg-cyan-700 hover:scale-105">
              <p className="flex font-medium items-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
        ...
        <div
          className="mmd:hidden rounded-full hover:bg-gray-200 p-2 cursor-pointer transition-all duration-300"
      ...
      {isMenuToggled && (
        <div className="flex flex-col mmd:hidden bg-white shadow-md absolute animate-slide-in right-0">
          {/* Link to hotel Route */}
          <Link to={"/hotels"}>
            <p className="flex font-medium items-center cursor-pointer px-4 py-3 hover:bg-gray-200 transition-colors duration-300">
              <svg viewBox="0 0 24 24" className="w-6 h-6 mr-2">
                <path d="M20.587 12.139V4.144H3.424v7.986A3.805 3.805 0 002 15.097v4.755h1.906v-1.905h16.188v1.91H22v-4.76a3.804 3.804 0 00-1.413-2.958zm-1.906-6.09V8.83a5.048 5.048 0 00-2.865-.876c-1.565 0-2.952.69-3.816 1.749-.864-1.059-2.252-1.749-3.818-1.749-1.07 0-2.056.324-2.851.866V6.049h13.35zm-.258 5.248c-.077-.005-.155-.012-.234-.012h-4.971c.438-.838 1.437-1.426 2.598-1.426 1.168 0 2.173.593 2.607 1.438zm-7.643-.012H5.812c-.081 0-.159.007-.238.012.434-.844 1.438-1.438 2.606-1.438 1.163 0 2.163.588 2.6 1.426zM3.906 16.04v-.943c0-1.051.855-1.905 1.906-1.905h12.376c1.051 0 1.905.854 1.905 1.905v.943H3.906z"></path>
              </svg>
              Hotels
          ...
          <Link to={"/map"}>
            <p className="flex font-medium items-center cursor-pointer px-4 py-3 bg-cyan-600 text-white hover:bg-cyan-700 transition-colors duration-300">
