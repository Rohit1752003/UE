      ...
      {result?.result_object?.category?.key === "geographic" ? (
        ...
        <div className="block md:grid md:grid-cols-12 p-4 gap-3 border-b">
          <div className="col-span-3 rounded overflow-hidden max-h-[200px] md:max-h-[150px] relative">
            ...
            <img
              src={
                result?.result_object?.photo
                  ? result?.result_object?.photo?.images?.large?.url ||
                    result?.result_object?.photo?.images?.original?.url ||
                    result?.result_object?.photo?.images?.medium?.url ||
                    result?.result_object?.photo?.images?.small?.url
            ...
            <p className="absolute text-xs z-20 bottom-0 text-white p-2">{result?.result_object?.category?.name}</p>
            {/* --- */}
          ...
          <div className="col-span-9 flex flex-wrap">
            <div className="w-full self-start">
              {/* Result Object Name */}
              <h3 className="font-semibold text-lg mb-2 mt-1 md:mt-0">{result?.result_object?.name}</h3>
              {/* --- */}
              ...
              <div className="text-sm text-dark">
                ...
                <p>{result?.result_object?.address || result?.result_object?.location_string}</p>
                {/* --- */}
              </div>
            </div>
            ...
            {result?.review_snippet && result?.review_snippet?.snippet && (
              <div className="w-full self-end text-xs mmd:text-sm text-dark mt-5 mmd:mt-0 space-y-1">
                ...
                <p>"...{result?.review_snippet?.snippet && result?.review_snippet?.snippet}..."</p>
              </div>
            )}
            {/* / Review Snippet Displayed If Available */}
          </div>
