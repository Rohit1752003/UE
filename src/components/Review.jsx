    ...
    <>
      <div className="flex flex-wrap md:grid md:grid-cols-12">
        <div className="w-full flex md:flex-col md:col-span-2 items-center">
          {/* Review User Picture */}
          <img src={review?.user?.avatar?.large?.url} className="w-10 h-10 md:w-14 md:h-14 object-cover rounded-full" />
          {/* --- */}

          {/* review User Username */}
          <p className="ml-3 md:ml-0 md:mt-2 text-xs text-center">{review?.user?.username}</p>
          {/* --- */}
        </div>
        ...
        <div className="w-full md:col-span-10 space-y-2 mt-2 md:mt-0">
          ...
          {review?.photos && (
            <div className="flex space-x-2">
              ...
              {review?.photos?.map((pic, i) => (
                <img key={i} src={pic?.images?.small?.url} className="w-20 h-16 object-cover my-1 cursor-pointer" />
              ))}
              {/* --- */}
            </div>
          )}
          ...
          {review?.owner_response && (
            <div className="pl-3 md:pl-4 border-l border-gray-300 my-2">
              {/* Responder Details */}
              <p className="text-sm">
                {review?.owner_response?.responder}, {review?.owner_response?.connection} at {place},{" "}
