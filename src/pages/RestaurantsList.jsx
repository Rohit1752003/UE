...
const RestaurantsList = () => {
  ...
  useEffect(() => {
    ...
    getPlacesByLatLng("restaurants", coordinates.lat, coordinates.lng, { limit: 30, min_rating: 4 }, source).then(
      (data) => {
    ...
    <>
      ...
      <div className="container mx-auto pb-4">
        <div className="text-center my-10">
          <h1 className="font-semibold text-lg md:text-3xl">Restaurants near you</h1>
        </div>
          ...
          <div className="lg:col-span-9">
            {!restaurants || isLoading ? (
              ...
              restaurants?.map((restaurant, i) => (
                <div
                  key={i}
                  className="grid grid-cols-12 my-2 border border-gray-200 rounded shadow hover:shadow-xl cursor-pointer transition-all duration-300 hover:-translate-y-1"
                >
                  {/* Restaurant Image */}
                  ...
                  <div className="col-span-12 md:col-span-8 px-2 md:px-4 py-2">
                    {/* Restaurant Name */}
                    <h2 className="font-semibold text-lg md:text-xl">{restaurant?.name}</h2>
                    {/* --- */}
                    ...
                    <p className="flex items-center my-1">
                      ...
                      <span className="text-sm mr-3">{restaurant?.num_reviews} Reviews</span>
                      {/* --- */}
                      ...
                      {!restaurant?.isClose && restaurant?.open_now_text && (
                        ...
                        <button
                          className={`${restaurant?.open_now_text?.includes("Closed") ? "bg-red-400" : "bg-green-400"} rounded-full py-1 px-2 md:px-3 my-1 shadow-md`}
                        >
                          <p className="text-sm flex items-center">
                            <svg
                              className="h-4 md:h-5 w-4 md:w-5 mr-1"
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
